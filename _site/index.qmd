...
apex(type = "pie", mapping = aes(x = image_s_video_s_included, y = n)) %>%
  ax_colors(c("#FF6347", "#4682B4", "#32CD32")) %>%
...
```{r}
like_real_dat <- full_data %>%  #View()
  filter(list_source %in% c("Politicians", "Parties")) %>% 

  filter(ai == 1)  %>% 
  filter(str_detect(nature_of_the_image_video,  "contains real-looking conten")) %>% 

  mutate(nature_of_the_image_video = case_when(
    str_detect(nature_of_the_image_video, "contains real-looking conten") ~ get_text("like_real", lang),
    str_detect(nature_of_the_image_video, "does NOT") ~ get_text("like_unreal", lang)
  )) %>% 

  count(nature_of_the_image_video) %>% 


apex(type = "radialBar", mapping = aes(x = get_text("like_real", lang), y = n)) %>%
  ax_colors(c("#FA0347", "#AC0999", "#darkblue")) %>%
  ax_chart(height = 299) %>%  # Increase chart size
  ax_plotOptions(
    radialBar = list(
      hollow = list(size = "60%"),  # Adjust hollow size for more text space
...
htmltools::tagList(
  ...
  style = "
    ...
    padding: 10px;
  ",
  htmltools::tags$div(style = "flex: 1; min-width: 300px;", medienformatte),
  htmltools::tags$div(style = "flex: 1; min-width: 300px;", like_real_dat)
)
  )
)
...
like_real_dat <- full_data %>%  #View()
  ...
  filter(ai == 1)  %>% 
  drop_na(nature_of_the_image_video) %>% 
  mutate(nature_of_the_image_video = case_when(
    str_detect(nature_of_the_image_video, "contains real-looking conten") ~ get_text("like_real", lang),
    str_detect(nature_of_the_image_video, "does NOT") ~ get_text("like_unreal", lang)
  )) %>% 
...
apex(type = "donut", mapping = aes(x = nature_of_the_image_video, y = n)) %>%
  ax_colors(c("#FA0347", "#AC0999", "#darkblue")) %>%
  ax_chart(height = 299) %>%  # Increase chart size
  ax_plotOptions(
    radialBar = list(
