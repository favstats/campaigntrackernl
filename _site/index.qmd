...
create_political_treemap_ac2 <- function(
  ...
  # Create button HTML with simpler show/hide logic
  button_html <- tags$div(
    ...
    lapply(names(filter_labels), function(val) {
      tags$button(
        ...
        onclick = sprintf(
          "document.querySelectorAll('.chart-%s').forEach(el => el.style.display='none'); document.getElementById('chart-%s-%s').style.display='block'; document.querySelectorAll('.filter-btn-%s').forEach(btn => {btn.classList.remove('active'); btn.style.background='#fff'; btn.style.color='#333';}); this.classList.add('active'); this.style.background='#007bff'; this.style.color='#fff';",
  ...
  # Wrapper container to prevent height issues
  wrapper_style <- sprintf(
    "position: relative; height: %spx; overflow: hidden;",
    height
  )
  
  # Create chart containers with absolute positioning to prevent stacking
  chart_divs <- lapply(names(charts), function(val) {
    tags$div(
      id = paste0("chart-", container_id, "-", val),
      class = paste0("chart-", container_id),
      style = sprintf(
        "%s position: absolute; top: 0; left: 0; width: 100%%;",
        if(val != "all") "display: none;" else ""
      ),
      charts[[val]]
    )
  })
