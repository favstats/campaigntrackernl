...
# --- pick the default (largest) in R so there is always a label initially ----
stopifnot(is.numeric(proportions$value))
imax   <- which.max(proportions$value)
lab0   <- proportions$name[imax]
val0   <- round(proportions$value[imax])

woki <- apex(
  data = proportions, type = "radialBar",
  mapping = aes(x = name, y = value)
) %>%
  ax_labels(proportions$name) %>%           # be explicit about labels
  ax_chart(
    height = 420,
    sparkline = list(enabled = TRUE),
    tooltip = list(enabled = TRUE),
    events = list(
      # When chart mounts: store the "largest" as base and show it in the center
      mounted = JS("
        function(chart, config){
          var totals = chart.w.globals.seriesTotals;
          var labels = chart.w.config.labels;
          var maxIdx = totals.indexOf(Math.max.apply(null, totals));
          chart.el.__baseRadial = { idx: maxIdx, label: labels[maxIdx], value: totals[maxIdx] };
          chart.updateOptions({
            plotOptions:{ radialBar:{ dataLabels:{ total:{
              show: true,
              label: labels[maxIdx],
              formatter: function(){ return Math.round(chart.el.__baseRadial.value) + '%'; }
            }}}}
          }, false, true);
        }
      "),
      # Hover: show hovered slice
      dataPointMouseEnter = JS("
        function(event, chart, opts){
          var i   = opts.seriesIndex;
          var val = chart.w.globals.seriesTotals[i];
          var lab = chart.w.config.labels[i];
          chart.updateOptions({
            plotOptions:{ radialBar:{ dataLabels:{ total:{
              show: true,
              label: lab,
              formatter: function(){ return Math.round(val) + '%'; }
            }}}}
          }, false, true);
        }
      "),
      # Leave: restore the largest slice
      dataPointMouseLeave = JS("
        function(event, chart, opts){
          var b = chart.el.__baseRadial;
          if(!b) return;
          chart.updateOptions({
            plotOptions:{ radialBar:{ dataLabels:{ total:{
              show: true,
              label: b.label,
              formatter: function(){ return Math.round(b.value) + '%'; }
            }}}}
          }, false, true);
        }
      ")
  ...
  ax_plotOptions(
    radialBar = radialBar_opts(
      ...
      dataLabels = list(
        ...
        value = list(show = TRUE, formatter = JS("v => v + '%'"),
                     style = list(color = "#000", fontSize = "16px")),
        # Fallback initial center (shown before 'mounted' fires)
        total = list(
          show = TRUE,
          label = lab0,
          formatter = JS(sprintf("function(){ return %d + '%%'; }", val0))
        )
      )
    )
  ) %>%
