...
```{r}
# Simpler, stricter apexcharter treemap with optional list_source filter buttons
create_political_treemap_ac2 <- function(
  data,
  party_colors,
  ...
  make_chart <- function(chart_data) {
    # Sort by y (descending) so largest chunks are on the left
    chart_data <- chart_data[order(-chart_data$y), ]
    
    cols <- unname(party_colors[match(chart_data$x, names(party_colors))])
  ...
  enable_filter <- !is.null(filter_var) && filter_var %in% names(data)
  
  if (!enable_filter) {
    # Return plain chart (backwards compatible) - sorted
    return(make_chart(data))
  }
  
  # Filtering enabled - create multiple charts
  data$filter_group <- as.character(data[[filter_var]])
  filter_values <- unique(data$filter_group)
  
  if (is.null(filter_labels)) {
    filter_labels <- setNames(
      c("All", filter_values),
      c("all", filter_values)
    )
  }
  ...
  # Generate unique IDs
  container_id <- paste0("treemap_", sample(10000:99999, 1))
  
  # Create charts for each filter (each will be sorted internally by make_chart)
  charts <- list()
  charts[["all"]] <- make_chart(data)
  for (val in filter_values) {
    charts[[val]] <- make_chart(data[data$filter_group == val, ])
  }
  
  # Updated onclick that handles positioning properly
  make_onclick <- function(val) {
    sprintf(
      "document.querySelectorAll('.chart-%s').forEach(el => {
        el.style.display='none'; 
        el.style.position='absolute'; 
        el.style.visibility='hidden';
      }); 
      var targetChart = document.getElementById('chart-%s-%s');
      targetChart.style.display='block'; 
      targetChart.style.position='relative'; 
      targetChart.style.visibility='visible';
      document.querySelectorAll('.filter-btn-%s').forEach(btn => {
      ...
      container_id, container_id, val, container_id
    )
  }
  
  # Create button HTML with improved show/hide logic
  button_html <- tags$div(
    class = "treemap-filter-buttons",
    style = "text-align: center; margin-bottom: 15px;",
    lapply(names(filter_labels), function(val) {
      tags$button(
        class = paste0("filter-btn-", container_id, if(val == "all") " active" else ""),
        onclick = make_onclick(val),
        style = paste0(
          "margin: 0 5px; padding: 8px 16px; ",
          "border: 1px solid #ddd; border-radius: 4px; ",
          "background: ", if(val == "all") "#007bff" else "#fff", "; ",
          "color: ", if(val == "all") "#fff" else "#333", "; ",
  ...
  # Create chart containers - initially all absolute except the first
  chart_divs <- lapply(names(charts), function(val) {
    is_first <- (val == "all")
    tags$div(
      id = paste0("chart-", container_id, "-", val),
      class = paste0("chart-", container_id),
      style = if(!is_first) {
        "display: none; position: absolute; top: 0; left: 0; width: 100%; visibility: hidden;"
      } else {
  ...
  # Return combined widget with wrapper
  htmltools::tagList(
    button_html,
    tags$div(
      style = wrapper_style,
...
# Example aggregation: count AI==1 posts by party, keeping list_source
treemap_df <- full_data %>%
  filter(ai == 1) %>%
  mutate(list_source = if_else(list_source %in% c("Parties","Politicians"),
                               "Parties", "Other Actors")) %>%
  count(party, list_source, name = "y") %>%
  transmute(x = party, y, list_source)
...
#   title = get_text("ai_by_party", lang),
#   height = 400,
#   list_source_filter = "buttons"     # <- add buttons; omit for old behavior
# )
# htmltools::browsable(tm2)

# tm2_filtered <- create_political_treemap_ac2(
#   data = treemap_df,
#   title = get_text("ai_by_party", lang),
#   party_colors = party_colors,
#   height = 400,
#   filter_var = "list_source",  # Enable filtering on this column
#   filter_labels = c(
#     "all" = "All",
#     "Parties" = "Parties",
...
#   tags$div(
#     class = "apex-container2",
#     htmltools::browsable(tm2_filtered)
#   )
# )
```
...
# Create chart
p1 <- create_political_treemap_ac(
  data = treemap_all,
  party_colors = party_colors,
  title = get_text("ai_by_party", lang),
  height = 400
...
```{r}
# Prepare data for "Parties"
treemap_parties <- full_data %>%
  filter(ai == 1, list_source %in% c("Parties", "Politicians")) %>%
  count(party, name = "y") %>%
  arrange(desc(y)) %>%
  transmute(x = party, y)

# Create chart
p2 <- create_political_treemap_ac(
  data = treemap_parties,
  party_colors = party_colors,
  title = get_text("ai_by_party2", lang),
  height = 400
)
...
```{r}
# Prepare data for "Other Actors"
treemap_others <- full_data %>%
  filter(ai == 1, !list_source %in% c("Parties", "Politicians")) %>%
  count(party, name = "y") %>%
  arrange(desc(y)) %>%
  transmute(x = party, y)
...
# Create chart
p3 <- create_political_treemap_ac(
  data = treemap_others,
  party_colors = party_colors,
  title = get_text("ai_by_party3", lang),
  height = 400
)
...
"))
 ...
 platform_colors = c("Facebook" = "#1877F2", "Instagram" = "#E4405F", "TikTok" = "#69C9D0", "X" = "000000")
# Prepare data for "All"
treemap_all <- full_data %>%
  filter(ai == 1) %>%
...
p1 <- create_political_treemap_ac(
  ...
  title = get_text("by_platform_all", lang),
  height = 400
)
...
p2 <- create_political_treemap_ac(
  data = treemap_parties,
  party_colors = platform_colors,
  title = get_text("by_platform_parties", lang),
  height = 400
)
...
p3 <- create_political_treemap_ac(
  data = treemap_others,
  party_colors = platform_colors,
  title = get_text("by_platform_other_actors", lang),
  height = 400
)
...
#' Create a media format (image/video) chart
#'
#' @param data Full dataset (will be filtered internally as needed)
#' @param lang Language code for translations (passed to get_text)
#' @param chart_height Height of the chart in pixels (default: 200)
#' @param img_color Color for image bars (default: "#FF6347")
#' @param vid_color Color for video bars (default: "#4682B4")
#' @param custom_css Optional CSS for styling
#'
#' @return An apexcharter plot wrapped in HTML tags
create_media_format_chart <- function(data, 
                                       lang, 
                                       chart_height = 200,
                                       img_color = "#FF6347",
                                       vid_color = "#4682B4",
  ...
  # CHANGED: Map colors by actual type content (not order)
  cats <- med_df$type
  colors_vec <- ifelse(cats == img_lbl, img_color,
                 ifelse(cats == vid_lbl, vid_color, "#9E9E9E"))
  ...
  medienformatte_bar <- med_df %>%
    ...
    ax_colors(colors_vec) %>%  # CHANGED: per-point colors mapped by label
    ax_legend(
      show = TRUE,
      position = "bottom",
      customLegendItems = c(img_lbl, vid_lbl),
      markers = list(fillColors = c(img_color, vid_color)),
      labels = list(colors = "#000")
    ) %>%
    ax_title(
      text = get_text("title_type", lang), 
      align = "center",
      style = list(fontSize = "18px", fontWeight = "bold")
    ) %>%
    ax_tooltip(
      ...
      custom = JS(sprintf("
        function({series, seriesIndex, dataPointIndex, w}) {
          var lab = w.globals.labels[dataPointIndex];
          var type = lab.split(' (n=')[0];
          var col = (type === %s) ? '%s' : (type === %s ? '%s' : '#9E9E9E');
          ...
          return '<div style=\"padding:6px 8px\">' +
                 ...
                 '<b>'+type+'</b></div>' +
                 '<div>'+Math.round(val*10)/10+'%%</div></div>';
        }",
        jsonlite::toJSON(img_lbl, auto_unbox = TRUE),
        img_color,
        jsonlite::toJSON(vid_lbl, auto_unbox = TRUE),
        vid_color
      ))
    )
  
  # Return wrapped chart
  tagList(
    custom_css,
    tags$div(
      class = "apex-container",
      browsable(medienformatte_bar)
    )
  )
}
...
full_data %>%
  filter(list_source %in% c("Politicians","Parties")) %>% 
  filter(ai == 1) %>%
  create_media_format_chart(lang = lang)
...
#' Create a tonality sentiment chart
#'
#' @param data Full dataset (will be filtered internally for ai == 1)
#' @param lang Language code for translations (passed to get_text)
#' @param exclude_sources Character vector of list_source values to exclude (default: c("Politicians", "Parties"))
#' @param custom_css Optional CSS for styling
#'
#' @return An apexcharter plot wrapped in HTML tags
create_tonality_chart <- function(data, 
                                   lang, 
                                   exclude_sources = c("Politicians", "Parties"),
                                   custom_css = NULL) {
  
  # Filter data
  filtered_data <- data 
  # Prepare negative tendency data
  negative_data <- filtered_data %>%
    count(negative_tendency_tonality) %>% 
    rename(tonalität = negative_tendency_tonality)
  
  # Prepare positive tendency data
  positive_data <- filtered_data %>%
    count(positive_tendency_tonality) %>% 
    rename(tonalität = positive_tendency_tonality)
  
  # Combine and clean data
  tonalitatdata <- positive_data %>% 
  ...
  # Create color vector based on categories
  cats <- tonalitatdata$tendenz_kurz
  colors_vec <- ifelse(cats == pos_lbl, green,
                 ifelse(cats == neg_lbl, red, gray))
  ...
  tonchart <- tonalitatdata %>% 
    ...
    ax_tooltip(
      ...
      custom = JS(sprintf("
        function({series, seriesIndex, dataPointIndex, w}) {
          var lab = w.globals.labels[dataPointIndex];
          var col = (lab === %s) ? '#F44336' : (lab === %s ? '#4CAF50' : '#9E9E9E');
          var val = series[seriesIndex][dataPointIndex];
          return '<div style=\"padding:6px 8px\">' +
                 '<div style=\"display:flex;align-items:center;gap:6px\">' +
                 '<span style=\"width:10px;height:10px;border-radius:50%%;background:'+col+'\"></span>' +
        ...
        jsonlite::toJSON(neg_lbl, auto_unbox = TRUE),
        jsonlite::toJSON(pos_lbl, auto_unbox = TRUE),
        get_text("theposts", lang)
      ))
    ) %>%
    ax_legend(
      position = "bottom",
      customLegendItems = c(pos_lbl, neg_lbl),
      markers = list(fillColors = c(green, red)),
      labels = list(colors = "#000")
    )
...
create_tonality_chart(tondat, lang = lang, custom_css = custom_css)
```
...
topic_map_en <- c(
  ...
  topic_constitution = "Constitutional Order",
