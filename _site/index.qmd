...
(function () {
  function poke() {
    // fire a few times to catch late layout/ font loads
    window.dispatchEvent(new Event('resize'));
    setTimeout(() => window.dispatchEvent(new Event('resize')), 100);
    setTimeout(() => window.dispatchEvent(new Event('resize')), 300);
  }
  document.addEventListener('shown.bs.tab', poke);
  document.addEventListener('shown.bs.collapse', poke);
  document.addEventListener('slid.bs.carousel', poke);
})();
</script>

<style>
/* Robust two-column layout without Bootstrap dependence */
.ac-grid {
  display: grid;
...
/* Make sure htmlwidgets have a real area even when just shown */
.ac-grid .html-widget { width: 100% !important; min-height: 420px; }
.ac-grid .apexcharts-canvas { margin: 0 auto; }
</style>
...
actor_type_colors <- c(
  "Politicians" = "#0072B2", # blue
  "Parties"     = "#D55E00", # vermillion
  "Fans"        = "#009E73"  # green
)
...
tm <- create_political_treemap_ac(
  ...
  title = get_text("by_type_text", lang),
      party_colors = c("Politicians" = "#0072B2", "Parties" = "#D55E00", "Fans" = "#009E73"),
  height = 400
)
...
disclaimer_overall <- apex(
  ...
  ax_theme(mode = "light")  # ensures consistent text/background  [oai_citation:2‡ApexCharts.js](https://apexcharts.com/docs/options/theme/?utm_source=chatgpt.com)
  ...
  ax_plotOptions(
    radialBar = radialBar_opts(
      ...
      dataLabels = list(
        ...
        total = list( # MUST be inside dataLabels (not sibling)  [oai_citation:4‡ApexCharts.js](https://apexcharts.com/docs/options/plotoptions/radialbar/?utm_source=chatgpt.com)
          show = TRUE,
          label = "Gesamt",
          formatter = JS("function(w){ return '100%'; }"),
          style = list(color = "#000", fontSize = "16px", fontWeight = "bold")
        )
      )
    )
  ) %>%
...
ac_two <- tagList(
  # Minimal CSS grid; works inside tabsets and anywhere Quarto renders HTML
  tags$style(HTML("
    .ac-grid {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(340px, 1fr));
    ...
    .ac-grid .html-widget { width: 100% !important; }  /* ensure widget fills its cell */
    .ac-grid .apexcharts-canvas { margin: 0 auto; }     /* tidy centering */
  ")),
  tags$div(
    class = "ac-grid",
    tags$div(disclaimer_overall),
    tags$div(woki)
  )
)

ac_two   # IMPORTANT: return only this, not the individual charts
```
...
intheshadows <- bytype_dis %>% filter(!(detect_bild == "TRUE" & detect_text == "TRUE" & detect_ad == "TRUE"))
